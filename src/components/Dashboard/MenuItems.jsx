...
export default function MenuItems({ item, isTitleHide }) {
  ...
  return (
    <Link
      ...
      className={`flex gap-3 xl:gap-4 items-center justify-center lg:justify-start rounded-md mx-1 sm:mx-[0.65rem] md:mx-[0.2rem] lg:mx-0 sm:py-2 md:py-4 md:px-2 lg:px-4 lg:py-3 p-1 ${
        isActive ? activeLink : ""
      }`}
      href={`/${item.path}`}
    >
      ...
      />
      <span
        className={`hidden ${isTitleHide ? "hidden" : "lg:block"} font-medium lg:text-[1.1rem] xl:text-xl ${
          isActive ? activeSpan : ""
        }`}
      >
        {item.title}
